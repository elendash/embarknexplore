...
export function Pages({ userAuth }) {
    ...
    return (
        <div className='flex flex-wrap'>
            < div className='bg-green-200 p-20 w-2/6 tracking-wide flex flex-col divide-y-2 > * + * divide-gray-700' >
                <h1 className='flex flex-col text-left text-gray-700 text-5xl font-sans pb-7 font-black uppercase tracking-wide'>{data.type}</h1>
                {
                    data.embark_n_explores.map((fullList, index) => (
                        <div key={index} className='flex flex-col pt-20 pb-20 pl-2 pr-24 text-xl text-gray-700 font-extrabold'>
                            ...
                            <div className='uppercase text-sm font-bold pt-16 pr-5'>{fullList.website === undefined ? "" : "Website:"}</div>
